...
def hisseyi_analiz_et(
    ...
    if veri_eski_mi(veri.index[-1], zaman_dilimi):
...
if not tara:
    ...
    # Özel listede yalnız 4-6 karakterli kodları kabul et.
    taranacaklar = [
        kod for kod in taranacaklar
    ...
    for grup_no, grup in enumerate(gruplar, start=1):
        ...
        for sembol in grup:
            ...
            ilerleme.progress(
                min(
                    int(tamamlanan / len(taranacaklar) * 100),
                    100,
                )
            )

        sonuc_alani.caption(
            f"Şu ana kadar {len(sonuclar)} güncel ve geçerli analiz bulundu."
        )
    ...
    if not sonuclar:
        st.error(
            "Güncel ve geçerli sonuç bulunamadı. Yahoo Finance geçici "
            "olarak veri göndermemiş olabilir. Önbelleği temizleyip yeniden dene."
        )
        st.stop()
    ...
    if ekran_tablosu.empty:
        st.warning(
            ...
            "Minimum AL puanını veya hacim çarpanını kademeli azalt."
        )
    else:
        st.subheader(f"📊 En güçlü {len(ekran_tablosu)} sonuç")
    ...
    if veri_alinamayanlar:
        with st.expander(
            ...
            st.write(", ".join(sorted(set(veri_alinamayanlar))))

    if eski_veriler:
        with st.expander(
            f"Eski verisi olan semboller: "
            f"{len(set(eski_veriler))}"
        ):
            st.write(", ".join(sorted(set(eski_veriler))))

    if yetersiz_veriler:
        with st.expander(
            f"Yetersiz verisi olan semboller: "
            f"{len(set(yetersiz_veriler))}"
        ):
            st.write(", ".join(sorted(set(yetersiz_veriler))))

    if dusuk_hacimliler:
        with st.expander(
            f"Minimum hacmin altında kalan semboller: "
            f"{len(set(dusuk_hacimliler))}"
        ):
            st.write(", ".join(sorted(set(dusuk_hacimliler))))
...
st.caption(
    ...
    "veya hatalı olabilir. Sinyaller gerçekleşmeden önce mutlaka "
    "grafik üzerinden kontrol edilmelidir."
)